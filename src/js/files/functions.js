...
export function textWriter() {
	document.addEventListener('DOMContentLoaded', function () { 
		let index = 0;
			const speed = 40; // Швидкість друку (мілісекунди)
			const textContainers = document.querySelectorAll('[data-textWriter]');
			const cursor = '<span class="cursor"></span>'; // Мигаючий курсор

			if (textContainers.length > 0) { // Перевірка на наявність елементів
				textContainers.forEach(el => {
					console.log(el.parentElement);
					if (el.parentElement.classList.contains('_watcher-view') && !el.classList.contains('finish')) {
						console.log('true');
						...
						function typeEffect() {
							if (index < text.length) {
								...
								index++;
								setTimeout(typeEffect, speed); // Викликаємо typeEffect після затримки
							} else {
								el.innerHTML = text;
								el.classList.add('finish');// Видаляємо курсор в кінці
							}
						}
						typeEffect(); // Викликаємо функцію для початку
